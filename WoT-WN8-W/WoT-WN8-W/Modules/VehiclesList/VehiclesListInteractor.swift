        ...
        if let json: NSDictionary = NSJSONSerialization.JSONObjectWithData(vehicleListData!, options: NSJSONReadingOptions.MutableContainers, error: nil) as? NSDictionary {
            if let items = json["data"] as? NSDictionary {
                for (key, value) in items {
                    ...
                    vehicleDataStruct.image = NSURL(string: (dict["image"] as! String))
                    vehicleDataStruct.image_small = NSURL(string: (dict["image_small"] as! String))
                    ...
                    vehicleDataStruct.contour_image = NSURL(string: (dict["contour_image"] as! String))
